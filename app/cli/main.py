...
Installed as the ``atlas`` console script.  This module lives inside the
``app`` package so it ships in the installed distribution (``tools/`` is
not packaged).
...
def main() -> None:
    ...
    Returns:
        ``0`` on success.  Error paths exit with status ``1``.
    """
    parser = argparse.ArgumentParser(
        description="Atlas CLI — developer tooling for the Atlas framework.",
    ...
    if args.command == "release-next":
        return cmd(args.level)
...
def cmd_list_vectorstores() -> int:
    ...
    try:
        ...
        # Inspect the built-in implementation rather than a hard-coded list.
        stores = [MemoryVectorStore.__name__]
        if not stores:
...
def cmd_validate_config(file_path: str) -> int:
...
def cmd_release_next(level: str = "patch") -> int:
...
def cmd_release_changelog(version: str | None = None) -> int:
