...
class ChunkingConfig:
    ...
    Attributes:
        strategy: The chunking strategy name (e.g. ``"fixed_size"``,
            ``"sentence"``, ``"paragraph"``).  Default ``"fixed_size"``.
        chunk_size: Maximum number of characters per chunk for
            ``fixed_size`` strategy.  Default 512.
        chunk_overlap: Number of characters to overlap between consecutive
            chunks for ``fixed_size``.  Default 64.
        ...
        window_size: Window size in characters for ``sliding_window``
            strategy.  Default 512.
        stride: Step size in characters for ``sliding_window`` strategy.
            Default 256.
